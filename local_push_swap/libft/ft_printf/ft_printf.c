...
int	string_function(int *cwrite, char *str)
{
	if (!str)
	{
		if (f_putstr_fd("(null)", 1) == -1)
			return (-1);
		*cwrite = *cwrite + 6;
		return (0);
	}
	if (f_putstr_fd(str, 1) == -1)
		return (-1);
	*cwrite = *cwrite + f_strlen(str);
	return (0);
}
...
static int	after_sign(int *cwrite, va_list args, char c)
{
	...
	temp = 1;
	if (c == 'c')
		temp = f_putchar_fd(va_arg(args, int), 1);
	if (c == 's')
		temp = string_function(cwrite, va_arg(args, char *));
	if (c == 'p')
		temp = print_address(va_arg(args, unsigned long long), cwrite);
	if (c == 'i' || c == 'd')
		temp = f_putnbr_count(cwrite, (va_arg(args, int)));
	if (c == 'u')
		temp = f_putunbr_count(cwrite, va_arg(args, unsigned int));
	if (c == 'x')
		temp = f_putnbr_base(va_arg(args, unsigned int), 0, cwrite);
	if (c == 'X')
		temp = f_putnbr_base(va_arg(args, unsigned int), 1, cwrite);
	if (c == '%')
		temp = f_putchar_fd('%', 1);
	if (c == 'c' || c == '%')
		*cwrite = *cwrite + 1;
